...
unsigned char try_next[MAX_CHOICES * 5], *try_end;
...
void print_state() {
    ...
    printf(" (%i to finish, %i pairs)\n", min_moves(curr_floor, floors), pairs(floors));
}
...
bool valid() {
    ...
    if ((el2 > el1) || (el1 > 127) || (ise2 > 1) || (ise1 > 1) || (direction == -3) ||
...
bool compute_path() {
    ...
    if ((sp - stack) / 4 + min_moves(curr_floor, floors) > max_steps)
        return false;
    print_state();
    try_end = try_next;
    valid_idx = 0;
    ...
    do {
        ...
        minmo = 4 * min_moves(next_floor, next_floors) + 4 - pairs(next_floors);
        ...
        try_end += 5;
        valid_idx++;
        if (valid_idx == MAX_CHOICES) { printf("too many choices"); exit(1); }
    } while (next());
    
    sp += 4;
    for(valid_idx=0; (valid_idx < 4) && (valid_idcs[valid_idx] >= 0); valid_idx++) {
