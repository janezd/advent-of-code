...
61 * (1 + 6 + 4) = 671 bytes + probably 61 * 4 = 244 bytes for calls.
...
Another, more efficient venue is to reimplement `stack` so that each call uses as
much memory for choices as it actually needs.
*/
...
void print_state() {
    ...
    for(t = (sp - stack) / ENC_LEN; t--; ) printf(" ");
    printf("%li ", (sp - stack) / ENC_LEN);
    printf("[[%i]]: ", curr_floor);    
