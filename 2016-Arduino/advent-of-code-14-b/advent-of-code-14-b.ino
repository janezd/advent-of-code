...
void recompute2016() {
    for(int i = 2016; i--; ) {
        MD5::make_digest(md5, md5b, 16);
        MD5::make_hash((char *)md5b, 32, md5);
    }
}
...
void setup() {
    pinMode(2, OUTPUT);
    int i, j, k, pptr3;
    for(i = 0;(nkeys < 64) || (i < maxkey + 1000); i++) {
        showNumber(i / 100, 6);
        sprintf(num, "%i", i);
        ...
        for(j = 0; j < MD5_DIGEST_LENGTH - 1; j++) {
            ...
            do {
                if (prev3[pptr3] == dig) {
                    int prevkeys;
                    for(prevkeys = 0; (prevkeys < nkeys) && (keys[prevkeys] != k); prevkeys++);
                    if (prevkeys == nkeys) {
                        keys[nkeys++] = k;
                        digitalWrite(2, HIGH);
                        showNumber(k, 0, 8);
